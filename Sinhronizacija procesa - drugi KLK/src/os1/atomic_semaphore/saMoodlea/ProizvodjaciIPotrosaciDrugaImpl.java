 ...
 * element. 
 */
public class ProizvodjaciIPotrosaciDrugaImpl extends Application {

	protected Bafer bafer = new Bafer(12);
	
	private AtomicInteger sto = new AtomicInteger(0);
	...
	protected class Bafer {
		...
		public void stavi(Element o) {
			...
			do {
				...
				ok = (nova >= 0 && nova < velicina);
				
				if(ok) {
					ok = sto.compareAndSet(stara, nova);	
				}
				if(!ok)
					Thread.yield();
