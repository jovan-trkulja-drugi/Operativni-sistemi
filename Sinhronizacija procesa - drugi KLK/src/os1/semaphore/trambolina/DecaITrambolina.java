...
public class DecaITrambolina extends Application {
	...
	protected final Trambolina trambolina = new Trambolina();
	...
	protected class Dete extends Thread {
		...
		protected void step() {
			...
			if(pol == Pol.MUSKI) {
				trambolina.popniDecaka(tezina);
				skace();
				trambolina.spustiDecaka(tezina);
			} else if(pol == Pol.ZENSKI) {
				trambolina.popniDevojcicu(tezina);
				skace();
				trambolina.spustiDevojcicu(tezina);
			}
		}
	}
